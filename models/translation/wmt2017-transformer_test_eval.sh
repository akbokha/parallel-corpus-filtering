...
experiment_dir=$3
data_dir=models/wmt2017-transformer-$SRC-$TRG/data/
...
do
    if [ ! -e "$output_dir/$prefix.$TRG.output" ]
    then
        cat $data_dir/$prefix.bpe.$SRC \
            ...
            > $output_dir/$prefix.$TRG.output
    fi
done

# calculate bleu scores on test sets
LC_ALL=C.UTF-8 $sacre_bleu/sacrebleu.py -t wmt14 -l $SRC-$TRG < $output_dir/test2014.$TRG.output > $output_dir/test2014.$TRG.output.bleu
LC_ALL=C.UTF-8 $sacre_bleu/sacrebleu.py -t wmt15 -l $SRC-$TRG < $output_dir/test2015.$TRG.output > $output_dir/test2015.$TRG.output.bleu
LC_ALL=C.UTF-8 $sacre_bleu/sacrebleu.py -t wmt16 -l $SRC-$TRG < $output_dir/test2016.$TRG.output > $output_dir/test2016.$TRG.output.bleu
LC_ALL=C.UTF-8 $sacre_bleu/sacrebleu.py -t wmt17 -l $SRC-$TRG < $output_dir/test2017.$TRG.output > $output_dir/test2017.$TRG.output.bleu
